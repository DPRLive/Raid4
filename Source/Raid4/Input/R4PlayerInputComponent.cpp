...
void UR4PlayerInputComponent::_InitializePlayerInput(UInputComponent* InPlayerInputComponent)
{
	...
	if ( UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>( InPlayerInputComponent ); IsValid( EnhancedInputComponent ) )
	{
		...
		// Player 스킬 입력 액션 바인딩
		for ( const TPair<EPlayerSkillIndex, TObjectPtr<UInputAction>>& skillAction : SkillActions )
		{
			// 스킬 index도 같이 넘긴다.
	...
	if ( UEnhancedInputLocalPlayerSubsystem* subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(
		playerController->GetLocalPlayer() ); IsValid( subsystem ) )
	{
 ...
 *  스킬 입력 시작
 */
void UR4PlayerInputComponent::_OnInputSkillStarted( const FInputActionValue& InValue, EPlayerSkillIndex InSkillIndex )
{
	if ( Owner.IsValid() )
 ...
 *  스킬 입력 중
 */
void UR4PlayerInputComponent::_OnInputSkillTriggered( const FInputActionValue& InValue, EPlayerSkillIndex InSkillIndex )
{
	if ( Owner.IsValid() )
 ...
 *  스킬 입력 종료
 */
void UR4PlayerInputComponent::_OnInputSkillCompleted( const FInputActionValue& InValue, EPlayerSkillIndex InSkillIndex )
{
	if ( Owner.IsValid() )
